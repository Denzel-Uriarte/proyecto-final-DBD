...
--     Al cerrar la estancia (fecha_hora_checkout_real cambia de NULL a valor), pasar la habitacion a 'limpieza'.
-- ---------------------------------------------------------------------
...
--     Cada vez que cambia el estado de una habitacion, dejar registro en bitacora_habitacion.
-- ---------------------------------------------------------------------
...
--     Cada vez que se registra un huesped facturador, agregarlo automaticamente a cliente_vip como 'bronce'.
-- ---------------------------------------------------------------------
...
--     Incrementa el contador personal del cliente VIP y mantiene numero_reservas en huesped_facturador.
...
--     Valida fecha_salida > fecha_inicio.
-- ---------------------------------------------------------------------
...
--     Control de inventario: bloquear check-in si la habitacion no esta 'disponible' o 'limpieza'.
-- ---------------------------------------------------------------------
...
--     Una vez al dia revisa reservas en estado 'pendiente' o 'confirmada' cuya fecha_inicio ya paso sin estancia asociada, y las marca como 'expirada'.
-- ---------------------------------------------------------------------
...
--     Evitar precios <= 0 en servicios y consumos.
-- ---------------------------------------------------------------------
...
--     Penalizacion 55% del subtotal de la reservacion si la cancelacion cae dentro de la "ventana de penalizacion" (< 7 dias antes del check-in).
--     Si cancela con anticipacion >= 7 dias, penalizacion 0.
--     Despues del INSERT marca la reservacion como 'cancelada' y libera la habitacion si ya estaba ocupada (caso raro: no-show).
-- ---------------------------------------------------------------------
...
-- Si por alguna razon la habitacion seguia ocupada (no-show real), la liberamos.
  UPDATE habitacion h
...
-- Ejecutar manualmente: SHOW TRIGGERS FROM hotel_alpheus; SHOW EVENTS  FROM hotel_alpheus;
